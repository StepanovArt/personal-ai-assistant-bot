...
def test_normalize_known_category():
    assert _normalize_category("еда") == "еда"


def test_normalize_synonym():
    assert _normalize_category("такси") == "транспорт"
    assert _normalize_category("продукты") == "еда"
    assert _normalize_category("аптека") == "здоровье"


def test_normalize_unknown_falls_back_to_другое():
    assert _normalize_category("космос") == "другое"


# ── unit: currency detection ───────────────────────────────────────────────────

def test_detect_dirham():
    assert _detect_currency("кола 3 дирхама") == "AED"


def test_detect_ruble():
    assert _detect_currency("такси 200 руб") == "RUB"
...
def test_regex_fallback_extracts_amount():
    result = _regex_fallback("такси 50 дирхам")
...
def test_regex_fallback_decimal():
    result = _regex_fallback("кофе 12.5 дирхама")
...
def test_extract_expense_valid_llm(monkeypatch):
    payload = json.dumps({
        ...
        "category": "транспорт", "description": "такси до офиса"
    })
...
def test_extract_expense_llm_returns_garbage_uses_fallback(monkeypatch):
    ...
    result = extract_expense("кофе 15 дирхам")
...
def test_extract_expense_llm_raises_uses_fallback(monkeypatch):
    ...
    result = extract_expense("продукты 100 руб")
...
def test_extract_expense_llm_zero_amount_uses_fallback(monkeypatch):
    ...
    result = extract_expense("купил 20 дирхам еды")
...
def test_extract_expense_synonym_category_normalized(monkeypatch):
    payload = json.dumps({
        ...
        "category": "ресторан", "description": "обед"
    })
    monkeypatch.setattr("agents.expense_agent.call_llm", lambda _: payload)

    result = extract_expense("ресторан 25")
